    ...
    if (T)
    {
        ...
        case ASSIGNOP:
            ...
            else
            {
                ...
                if (T->ptr[0]->type != T->ptr[1]->type && T->ptr[0]->kind != ARRAY_READ)
                {
...
{ //对抽象语法树的先根遍历,按display的控制结构修改完成符号表管理和语义检查和TAC生成（语句部分）
    ...
    if (T)
    {
        ...
        case CONTINUE:
            if (IN_LOOP == 0)
                semantic_error(T->pos, " ", "不在循环体内部，break/continue语句出现位置错误");
            break;
