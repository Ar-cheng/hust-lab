...
int array_read(struct ASTNode *T) //返回数据类型
{
    ...
    for (j = i - 2; j >= 0; j--)
    {
        ...
        T->place = fill_Temp(newTemp(), LEV, INT, 'T', offset);
        ...
        opn1.offset = symbolTable.symbols[Node_Array[j + 1]->place].offset;
        ...
        tmp = fill_Temp(newTemp(), LEV, INT, 'T', offset);
        ...
        result.kind = ID;
        result.offset = symbolTable.symbols[tmp].offset;

        opn2.kind = ID;
        strcpy(opn2.id, symbolTable.symbols[Node_Array[j]->place].alias);
        opn2.offset = symbolTable.symbols[Node_Array[j]->place].offset;
    ...
    tmp1 = fill_Temp(newTemp(), LEV, INT, 'T', offset);
...
{ //处理变量列表
    ...
    case ARRAY_DEF: //数组定义
        ...
        while (T0->kind == ARRAY_DEF)
        {
            if (T0->ptr[1]->type != INT)
                semantic_error(T->pos, "", "数组下标不符");
            if (T0->ptr[1]->type_int <= 0 || !strcmp(T->ptr[1]->type_id, "UMINUS"))
                semantic_error(T->pos, T->type_id, "数组下标不符");
        ...
        rtn = fillSymbolTable(T0->type_id, newAlias(), LEV, T->type, 'A', T->offset); //最后一个变量名

        if (rtn == -1)
            semantic_error(T->pos, T0->type_id, "变量名重复定义");
        else
        {
            T->place = rtn;
            T->num = 1;
        ...
        case ASSIGNOP:
            ...
            else
            {
                ...
                else if (T->ptr[0]->kind == ARRAY_READ)
                {
                    ...
                    opn2.kind = ID;
                    opn2.offset = symbolTable.symbols[symbolTable.symbols[T->ptr[0]->place].array_info->tartget].offset;
...
{ //对抽象语法树的先根遍历,按display的控制结构修改完成符号表管理和语义检查和TAC生成（语句部分）
    ...
    if (T)
    {
            ...
            { //处理所以DEC_LIST结点
                ...
                else if (T0->ptr[0]->kind == ARRAY_DEF)
                {
                    ...
                    if (array)
                    {
                        ...
                        //计算宽度
                        for (i = 0, total = 1; i < array->dimension; i++)
                        {
                            // printf("diminfo:%d\n", array->diminfo[i]);
                            total *= array->diminfo[i];
                        }

                        T->width += (array->type == CHAR ? 1 : 4) * total;
                        // printf("T->width:%d\n", T->width);
                    }
                }
                T0 = T0->ptr[1];
        ...
        case RETURN:
            ...
            {
                T->ptr[0]->offset = T->offset;
                // printf("return_debug:%d,%d",T->ptr[0]->kind);
                Exp(T->ptr[0]);
